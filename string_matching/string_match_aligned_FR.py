...
if __name__ == "__main__":    
    ...
    files_to_match = []
    ## aligned  
    #files_to_match.append('AlignedCauses_2006-2012full')
    #files_to_match.append('AlignedCauses_2013full')
    files_to_match.append('AlignedCauses_2014_full')
    ...
    for f in files_to_match:
        ...
        new_df = smatch.find_codes_in_text_aligned(df, icdcodes_strings, textf, stemming=True, ignorediacritics=True, removestopwords=True, lang='french')
        

        outfolder = os.path.join(folder,'string_matched_full_test')
        if not os.path.isdir(outfolder):
            os.mkdir(outfolder)

        outf_s = os.path.join(outfolder,f+'_system_list'+file_extension)
        outf_s2 = os.path.join(outfolder,f+'_system'+file_extension)
        smatch.print_with_list_aligned(new_df, training_code_freq_list, outf_s)
        smatch.print_line_by_line_for_eval_aligned(new_df, training_code_freq_list, outf_s2)
